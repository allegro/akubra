...
func TestDeleteResponsePickerSomeBad(t *testing.T) {
	...
	for _, serie := range responsesSeries {
		responsesChan := createChanOfResponses(serie...)
		delResponsePicker := &baseDeleteResponsePicker{BasePicker{responsesChan: responsesChan}, nil, nil, pullResponses}
...
func TestWatchogDeletePicker(t *testing.T) {
	responsesSeries := [][]bool{
	...
	for i, serie := range responsesSeries {
		responsesChan := createChanOfResponses(serie...)
		delResponsePicker := &baseDeleteResponsePicker{BasePicker{responsesChan: responsesChan}, nil, nil, pullResponsesWatchdog}
		resp, err := delResponsePicker.Pick()

		if responsesSeries[i][3] {
			require.NoError(t, err)
			require.NotNil(t, resp)
		} else {
			require.Error(t, err)
